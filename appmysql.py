...
# GRANT ALL PRIVILEGES ON *.* to 'root'@'8.219.158.94' identified by 'Investarget@2017' WITH GRANT OPTION;
...
def chatgptWithPDFFile():
    try:
        ...
        return msg
db_user = "root"
db_password = "Investarget@2017"
db_host = "192.168.0.1:3306"
db_name = "investarget"
sql_db = SQLDatabase.from_uri(f"mysql+pymysql://{db_user}:{urlquote(db_password)}@{db_host}/{db_name}?charset=utf8")

@app.route('/mysql/', methods=['POST'])
def chatgptWithMysql():
    try:
        chat_model = request.json.get('chat_model')
        OPENAI_API_KEY = request.json.get('open_ai_key')
        question = request.json.get('question')
        os.environ["OPENAI_API_KEY"] = OPENAI_API_KEY
        llm = ChatOpenAI(temperature=0, openai_api_key=OPENAI_API_KEY, model_name=chat_model)
        db_chain = SQLDatabaseSequentialChain.from_llm(llm=llm, db=sql_db, verbose=True, use_query_checker=True)
        result = db_chain.run(question)
        return {'success': True, 'result': result, 'errmsg': None, 'reset': True}
    except Exception:
        msg = {'success': False, 'result': None, 'errmsg': traceback.format_exc(), 'reset': True}
        print(traceback.format_exc())
        return msg
